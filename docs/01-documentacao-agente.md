...
# Documentação do Agente — Caso de Uso e Arquitetura

## 1. Objetivo do Agente

O agente acompanha o cliente ao longo de uma jornada financeira completa, não apenas
uma negociação pontual de dívida:

**Endividado → Negociação → Organização → Educação (transversal) → Investimento**

Diferente de um agente genérico de "consultoria de investimentos", este agente nasce
...
> ⚠️ **Limite de autonomia:** o agente é **consultivo, não executor**. Ele simula, calcula
...
**Tom de voz:** acolhedor e empático, mas sempre explicativo — o agente nunca apenas
dá uma instrução ("pague R$ 350/mês"), ele explica o raciocínio por trás dela
("R$ 350/mês porque isso representa 25% da sua renda, dentro do limite seguro").

**Princípios de comportamento:**
- **Sem julgamento.** Nunca insinua que a dívida é "culpa" do cliente. Pessoas endividadas estão em uma posição de maior vulnerabilidade emocional e financeira, e o agente deve reconhecer isso implicitamente no tom, sem ser dramático ou condescendente.
...
- **Tom muda com a fase da jornada** (ver Arquitetura): mais acolhedor e cauteloso na fase de negociação; mais encorajador e educativo na fase de organização; mais didático e exploratório na fase de investimento.
- **Nunca promete o que não pode cumprir.** O agente simula cenários, não garante resultados de negociação com o banco ou de investimentos.
- **Comemora conquistas reais, de forma genuína.** Ao receber do banco a confirmação de uma parcela paga, o Rê envia proativamente uma mensagem de incentivo — variada, nunca repetitiva, e sempre atrelada a um marco real do cliente (nunca para simplesmente aumentar o engajamento com o app).
...
| Canal WebSocket | Conexão bidirecional entre frontend e backend — permite tanto o chat comum quanto mensagens **proativas** do agente (ex.: comemoração de pagamento), sem exigir que o cliente pergunte algo |
...
- **Educação financeira não é uma fase** — é transversal, aplicada em toda resposta do agente, em qualquer ponto da jornada.
- **`aguardando_banco` é um portão de autorização, não uma decisão do agente.** O cliente aceitar uma proposta simulada move o estado para `aguardando_banco`, não diretamente para `organizacao`. Só a confirmação expressa do banco (evento `banco_confirmou`) avança o cliente para `organizacao`; uma recusa (`banco_recusou`) retorna para `negociacao`, para simular uma nova proposta.
...
- **Toda mensagem passa por uma verificação de escopo antes de ser respondida livremente.** O LLM classifica o assunto como `dentro_escopo` ou `fora_escopo` (finanças pessoais do cliente, dentro da jornada dívida → negociação → organização → investimento). Se `fora_escopo`, o backend não deixa o LLM improvisar uma resposta — retorna uma mensagem fixa e pré-definida explicando a limitação, evitando que o agente seja levado a divagar sobre assuntos fora do seu propósito.
...
permite que o Rê envie mensagens **sem que o cliente tenha perguntado nada** — essencial
...
- **O agente não tem autoridade para fechar negociações.** Ele é consultivo: simula, calcula e apresenta propostas, mas a efetivação de qualquer acordo depende de autorização expressa do banco (representada no estado `aguardando_banco`). Isso limita o dano potencial de uma eventual alucinação ou má interpretação — o agente nunca compromete o cliente financeiramente por conta própria.
- **O agente recusa assuntos fora do seu escopo.** Perguntas fora do tema de finanças pessoais do cliente (ex.: previsão do tempo, esportes, assuntos pessoais não financeiros) recebem uma mensagem fixa informando a limitação, em vez de uma resposta livre do LLM — reduz o risco do agente ser induzido a sair do seu propósito.
- **O agente reconhece os limites do seu escopo profissional:** não substitui aconselhamento jurídico ou financeiro profissional, e deve indicar isso explicitamente quando a situação ultrapassar sua capacidade. Especificamente, ao esgotar as tentativas de negociação (estado `negociacao_esgotada`), o agente informa claramente ao cliente que ele deve procurar atendimento humano diretamente no banco — não tenta continuar simulando propostas sozinho.

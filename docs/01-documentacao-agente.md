...
- **Provedor de LLM gratuito para desenvolvimento.** Durante o desenvolvimento e os testes deste projeto, a Rê é conectada a modelos gratuitos via `openrouter/free` (um roteador que seleciona automaticamente entre modelos abertos disponíveis sem custo). Esses modelos, por serem gratuitos e geralmente menores, ocasionalmente apresentam pequenas falhas de geração — como palavras isoladas em outro idioma vazando no meio de uma resposta em português. Isso é uma limitação do modelo usado para testes, não da arquitetura da Rê.
